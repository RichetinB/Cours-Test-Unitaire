...
class BookDAOIT(
    ...
    init {
        ...
        test("update book reserved flag in db") {
            // GIVEN
            performQuery("""
        insert into book (title, author, reserved)
        values ('Test', 'Author', false);
    """)

            val insertedId = performQuery("SELECT id FROM book").first()["id"] as Int
            ...
            // THEN
            val res = performQuery("SELECT * FROM book WHERE id = $insertedId").first()

            res["reserved"].shouldBe(true)
            res["title"].shouldBe("Test")
            res["author"].shouldBe("Author")
        }

        test("get books should include reserved flag") {
            performQuery("""
        insert into book (title, author, reserved)
        values ('Book1', 'A', true);
    """)
